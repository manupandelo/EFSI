...
let decenas=[1,10,20,30,40,50,60,70,80,90];
...
const ControlarDecena = (num) =>{
    for(let j=0;j<decenas.length;j++){
        if(num>=decenas[j] && num<=decenas[j+1]){
            decena=j;
        }
    }
    if(contadordecenas[decena]==2){
       return -1;
    }
    else{
